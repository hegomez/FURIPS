<div class="row row-bottom-margin">

    <div class="col-md-3">
        <div class="input-group">
            <div class="input-group-addon"><i class="fas fa-id-card"></i></div>
            <?php echo frmControl(44,'tooltip'); ?>
        </div>
    </div>
    <div class="col-md-5">
        <div class="input-group">
            <div class="input-group-addon"><i class="fas fa-address-card"></i></div>
            <?php echo frmControl(45,'placeholder'); ?>
        </div>
    </div>
    <div class="col-md-2">
        <button id="vic2prop" data-toggle="tooltip" data-placement="top" title="" data-original-title="Victima es Propietario" class="btn btn-default btn-block"><i class="fas fa-user-injured"></i></button>
    </div>
    <div class="col-md-6">
        <div class="input-group">
            <div class="input-group-addon"><i class="fas fa-user"></i></div>
            <?php echo frmControl(46,'placeholder'); ?>
        </div>
    </div>
    <div class="col-md-6">
        <div class="input-group">
            <div class="input-group-addon"><i class="fas fa-user"></i></div>
            <?php echo frmControl(47,'placeholder'); ?>
        </div>
    </div>
    <div class="col-md-6">
        <div class="input-group">
            <div class="input-group-addon"><i class="fas fa-user"></i></div>
            <?php echo frmControl(48,'placeholder'); ?>
        </div>
    </div>
    <div class="col-md-6">
        <div class="input-group">
            <div class="input-group-addon"><i class="fas fa-user"></i></div>
            <?php echo frmControl(49,'placeholder'); ?>
        </div>
    </div>
    

    <div class="col-md-7">
        <div class="input-group">
            <div class="input-group-addon"><i class="fas fa-map-signs"></i></div>
            <?php echo frmControl(50,'placeholder'); ?>
        </div>
    </div>
    <div class="col-md-5">
        <div class="input-group">
            <div class="input-group-addon"><i class="fas fa-phone"></i></div>
            <?php echo frmControl(51,'placeholder'); ?>
        </div>
    </div>
    <div class="col-md-6">
        <div class="input-group">
            <div class="input-group-addon"><i class="fas fa-map-marker-alt"></i></div>
            <?php echo frmControl(52,'placeholder'); ?>
        </div>
    </div>
    <div class="col-md-6">
        <div class="input-group">
            <div class="input-group-addon"><i class="fas fa-map-marker-alt"></i></div>
            <?php echo frmControl(53,'placeholder'); ?>
        </div>
    </div>
</div>